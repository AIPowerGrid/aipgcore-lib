Contributing to aipgcore
=======

We're working hard to make *aipgcore* the most powerful JavaScript library for working with aipgcoin. Our goal is to have *aipgcore* be a library that can be used by anyone interested in aipgcoin, and to level expertise differences with great design and documentation.
...
These are some global design goals in aipgcore that any change must adhere.
...
When possible, aipgcore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of aipgcore methods. For example, calling `JSON.stringify` on a aipgcore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'aipgcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the aipgcore object.
...
```sh
git remote add underdarkskies git@github.com:AIPowerGrid/aipgcore.git
...
```
Finally go to [github.com/AIPowerGrid/aipgcore](https://github.com/AIPowerGrid/aipgcore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of aipgcore.

If you have any questions feel free to post them to
[github.com/AIPowerGrid/aipgcore/issues](https://github.com/AIPowerGrid/aipgcore/issues).
